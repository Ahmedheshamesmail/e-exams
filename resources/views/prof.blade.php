...
<html>
    ...
    <a href="Login" style="position: absolute;right: 50px;top: 10px;">
        <button class="btn btn-block">Login</button>
    </a>
    <hr>
    <center style="padding-top: 20vh">

                <form action="insertprof" class="pure-form pure-form-stacked" method="post" style="padding: 15px;height: auto;width: 280px;background-color: white;box-shadow: -5px 2px 20px 2px gray;border-radius: 20px;">
           {{csrf_field()}}
                    <fieldset>
                        <legend>Welcome Professor <i class="fas fa-smile" style="color: rgba(230,217,31,0.96);"></i> </legend>
                        <div class="pure-control-group">
                            <input id="name" type="text" name="name" placeholder="Enter your Name ..." style="padding-bottom: 10px" value="{{old('name')}}">
                            @error('name')
                            <div style="color: red;font-size: 13px;padding-top: -10px">{{$message}}</div>
                            @enderror
                        </div>
                        <input type="text" name="phone" placeholder="Enter your phone ..." style="padding-bottom: 10px" value="{{old('phone')}}">
                        @error('phone')
                        <div style="color: red;font-size: 13px;padding-top: -10px">{{$message}}</div>
                        @enderror
                        <input type="password" name="password" placeholder="password" style="padding-bottom: 10px">
                        @error('password')
                        <div style="color: red;font-size: 13px;padding-top: -10px">{{$message}}</div>
                        @enderror
