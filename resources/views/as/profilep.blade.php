                            ...
                            @if($o->subject_id == $s->id)
                                @foreach($rowq as $q)
                                    ...
                                    <span style="font-size: 18px">
                                        الماده : <span style="color: #006666;font-size: 16px">{{$s->name}}</span>
                                    </span><br>
                                    @foreach($rowd as $d)
                                        @if($d->id == $s->department_id)
                                            <span style="font-size: 18px">
                                                القسم : <span style="color: #006666;font-size: 16px">{{$d->name}}</span>
                                            </span><br>
                                            @foreach($rowl as $l)
                                                @if($l->id == $d->level_id)
                                                <span style="font-size: 18px">
                                                           الصف : <span style="color: #006666;font-size: 16px">{{$l->name}}</span>
                                                </span><br>
                                                @endif
                                            @endforeach
                                        @endif
                                    ...
                                    <div style="border: 2px solid blueviolet;padding: 5px;border-radius: 5px;padding:10px">
                                        <h5 style="border-bottom: 1px solid #cccccc;width: 200px">اجابة الطالب</h5> <i class="fas fa-arrow-circle-right"></i>
                                        <span>
                                            <i> {{$o->text}}</i>
                                        </span>
                                    </div>
                                        ...
                                        <hr>
                                        <form action="NewRanka" method="post">
                                            {{csrf_field()}}
                                            <input class="form-control" type="number" name="degree" placeholder="الدرجه .." style="width: 200px">
