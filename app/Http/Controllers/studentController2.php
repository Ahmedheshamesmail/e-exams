...
class studentController2 extends Controller
{
    ...
    public function index()
    {
            ...
            if(count($timeroow)>0){
                foreach ($timeroow as $t){
                    ...
                    if(count($timeopening)>0){
                        ...
                        return redirect()->back()->with('nulls', 'لم يعد الاختبار متوفرا');
                    }
                }
            }
        ...
        if(count($rowoes) <1){
           ...
           if(count($rowoes) >0){
               foreach ($rowoes as $os){
                    if($os->work == 0){
                        return redirect()->back()->with('nulls', 'لقد قمت بإجتياز ها الاختبار من قبل !!');
                    }if($os->work == 1){
     ...
     return view('as.studentexam',compact('time','student','rowstruc','rowq','rowans','rowd','rows','s','rowt','rowp','subject','rowl','da2','rowat'));
    }
        ...
        $rowFinalOpen = DB::table('openquestions')
            ...
            DB::table('openexamstudents')
                ...
                if(count($rowFinalOpen )>0){
                    ...
                    ->where('subject_id',$subject)
                    ->delete();
                    return redirect('Studenta')->with('nulls','سيتم مراجعة اجاباتك');
    ...
    public function store(Request $request)
    {
        ...
        if($afterN !== $name){
            return redirect()->back()->with('nulls','لقد قمت بإدخال بيانات خاطئه برجاء ادخال بيانات صحيحه !!');
        }
    ...
    ->get();
    if(count($rowstu)>0){
        return redirect()->back()->with('nulls','عفوا , تم التسجيل من قبل بهذا الرقم القومي لطالب اخر !!');
    }
...
$t->save();
return redirect()->back()->with('nulls','تم اضافة طالب!!');
