...
class studennt2 extends Controller
{
    ...
    $rowopen = DB::table('openquestions')
            ->get();
     return view('as.profilep', compact('row','rowl','imr','rowq','rowd','rows','rowstu','rowopen'));
    }
    public function editrank(Request $req){
        ...
        return redirect()->back()->with('nulls','تم حفظ الدرجه !');
    }
    public function stuv( student $student,Request $request){
        ...
        if($afterI !== $id){
            return redirect()->back()->with('nulls','رقم الطالب التي ادخلته غير مفهوم الرجاء المحاوله مره اخري !!');
        }
        ...
        if($afterP !== $password){
            return redirect()->back()->with('nulls','كلمة السر التي ادخلتها غير مفهومه الرجاء المحاوله مره اخري !!');
        }
        else{
        ...
        if (count($student)<=0)
        {
            return redirect()->back()->with('nulls','البيانات التي ادخلتها غير متطابقه !!');
        }
        if (count($student)>0)
        {
            foreach ($student as $s){
                if(password_verify($password,$s->password)) {
                    ...
                    session()->put('pCovered', $request->password);

                    return redirect('Studenta');
                }else{
                    return redirect()->back()->with('nulls', 'كلمة السر غير صحيحه');
                }
            }
        }
    }
    ...
    public function stuv2( student $r,Request $request){
        ...
        $afterI = filter_var($beforeI,FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        if($afterI !== $nid){
            return redirect()->back()->with('nulls','الرقم القومي التي ادخلته غير مفهوم الرجاء المحاوله مره اخري !!');
        }
        else{
        ...
        if (count($student)<1)
        {
            return redirect()->back()->with('nulls','يرجى التأكد من الرقم القومي !!');
        }
        if (count($student)>0)
        {
                                                ...
                                                foreach ($rowl as $ll){
                                                    ...
                                                    if (count($rowt)>0){
                                                        foreach ($rowt as $tt){
                                                            ...
                                                            $t->time = $time;
                                                            $t->save();
                                                            return redirect()->back()->with('nulls',$sub2->name .' '.'بدء الامتحان');
...
//------------------------//
      ...
      return view('as.studentinfo',compact('rowstu','rowl','rowd','rows','rowopen','rowr','rowc','rows2','finalRating','finalRank'));
    ...
    public function openexam(){
        ...
        $rowo = DB::table('openexams')
            ->get();
        return view('as.openingexam',compact('row','imr','y','rows','rowd','rowp','rowl','rowo'));
        }
    public function substart(subject $s){
        ...
        $r2 = DB::table('openexams')
            ...
            ->get();
        if (count($r)>0){
            return redirect()->back()->with('nulls',$s->name .' الامتحان متاح بالفعل!!');
        }
        elseif (count($r2)>0){
            //if not open it//
             DB::table('openexams')
            ...
            return redirect()->back()->with('nulls',$s->name .' '.'بدء الامتحان');
        }
        //if exam is opened//
        else{
            $rowd = DB::table('departments')
            ...
            foreach ($rowd as $dd){
                ...
                foreach ($rowl as $ll){
                    ...
                    if (count($rowt)>0){
                        ...
                        return redirect()->back()->with('nulls','لم يتم تخصيص وقت الامتحان بعد!!');
                    }
                }
            }
        }
    ...
    public function substop(subject $s){
        ...
        if (count($r)>0){
            ...
            DB::table('datas')
               ->where('subject_id',$s->id)
               ->delete();
            return redirect()->back()->with('nulls',$s->name .' '.'تم ايقاف الامتحان');
        }
        else return redirect()->back()->with('nulls','للأسف,'.$s->name .' '.'لم يتم بدء الامتحان بعد !!');
    }
